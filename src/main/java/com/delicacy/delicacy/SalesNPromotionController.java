...
public class SalesNPromotionController implements Initializable {
    ...
    private void initLineChart()
    {
        ...
        try {
            ...
            rstA =stmtA.executeQuery(sqlB);
            while (rstA.next())
            series.getData().add(new XYChart.Data("Week 2",rstA.getInt(1)));

            rstA =stmtA.executeQuery(sqlC);
            while (rstA.next())
            series.getData().add(new XYChart.Data("Week 3",rstA.getInt(1)));

            rstA =stmtA.executeQuery(sqlD);
            while (rstA.next())
            series.getData().add(new XYChart.Data("Week 4",rstA.getInt(1)));
    ...
    private void initLineChart1()
    {
        ...
        try {
           ...
           series.getData().add(new XYChart.Data("Month 6",rstA.getInt(1)));



            rstA =stmtA.executeQuery(sqlE);
            while (rstA.next())
                series.getData().add(new XYChart.Data("Month 5",rstA.getInt(1)));

            rstA =stmtA.executeQuery(sqlD);
            while (rstA.next())
                series.getData().add(new XYChart.Data("Month 4",rstA.getInt(1)));

            rstA =stmtA.executeQuery(sqlC);
            while (rstA.next())
                series.getData().add(new XYChart.Data("Month 3",rstA.getInt(1)));

            rstA =stmtA.executeQuery(sqlB);
            while (rstA.next())
                series.getData().add(new XYChart.Data("Month 2",rstA.getInt(1)));


            rstA =stmtA.executeQuery(sqlA);
            while (rstA.next())
                series.getData().add(new XYChart.Data("Last Month",rstA.getInt(1)));
